...
//    private ChatInstructionCode CODE = new ChatInstructionCode();
    ...
    public ChatShowInputUI(String str_Id, String str_name, int type) {
        this.str_Id = str_Id;
    ...
    public void singleUI() {
        ...
        emoji_label.setIcon(new ImageIcon(ChatShowInputUI.class.getResource("/TalkSocket/GUI/images/emoji.png")));
        chatToolLeftPanel.add(emoji_label);

        JLabel screenshot_label = new JLabel("");
        screenshot_label.setIcon(new ImageIcon(ChatShowInputUI.class.getResource("/TalkSocket/GUI/images/screenshot.png")));
        chatToolLeftPanel.add(screenshot_label);

        JLabel file_label = new JLabel("");
        file_label.setIcon(new ImageIcon(ChatShowInputUI.class.getResource("/TalkSocket/GUI/images/file.png")));
    ...
    public void getInputAreaText(int type) {
        ...
        if(!str.equals(null)){
            // 私聊 = 0 & 群聊 = 1
            if (type == 0) {
                ClientAPI.send_msg(ClientAPI.u.uid + "",this.str_Id, str);
                System.out.println("发送私聊消息：当前id" + ClientAPI.u.uid +"发送id：" +  this.str_Id + "内容:" + str);
            }else if (type == 1) {
                ClientAPI.send_msg(ClientAPI.u.uid + "","ALL", str);
                System.out.println("发送群聊消息：当前id" + ClientAPI.u.uid +"发送id：" +  this.str_Id + "内容:" + str);
            }
            this.setShowAreaText("===>发送消息<===" + getTime() + "\n" + str);
